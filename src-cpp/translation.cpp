...
int translate_(void){
  ...
  if(strcmp(nodes[0].op, "") == 0){
    if(isDigit(nodes[0].l)){
      ...
      add_op("load", "r0", std::to_string(get_var_addr(nodes[0].l)));
    }
    ...
    for(i = 0; i <= node_count; i++){
      ...
      if(isDigit(nodes[i].l))
      {
        ...
        if(const_num < -32768 || 32768 < const_num){
          ...
          // Output2("loadi\tr%d, %d\n", register_, const_num);
          add_op("loadi", "r"+std::to_string(register_), std::to_string(const_num));
        }
        ...
        //        if(addr)
        // Output2("load\tr%d, %d\n", register_, addr);
        add_op("load", "r"+std::to_string(register_), std::to_string(addr));
      ...
      if(strcmp(nodes[i].op, "+") == 0){
        if(isDigit(nodes[i].r)){
          ...
          if(const_num < -32768 || 32768 < const_num){
            ...
            add_op("add", "r0", "label"+std::to_string(label_counter));
            label_counter++;
          }
          else{
            // Output2("addi\tr0,%d\n", atoi(nodes[i].r));
            add_op("addi", "r0", nodes[i].r);
          }
        }
        else
        {
          ...
          //         if(addr)
          // Output2("add\tr0,%d\n", addr);
          add_op("add", "r0", std::to_string(addr));
          //          else
          //            Output2("a");
        }
      }
        ...
        if(isDigit(nodes[i].r)){
          ...
          add_op("sub", "r0", "label"+std::to_string(label_counter));
            label_counter++;
          }
          else{
            // Output2("subi\tr0,%d\n", atoi(nodes[i].r));
            add_op("subi", "r0", nodes[i].r);
          }
        }
        else{
          // Output2("sub\tr0,%d\n", get_var_addr(nodes[i].r));
          add_op("sub", "r0", std::to_string(get_var_addr(nodes[i].r)));
        }
      }
      else if(strcmp(nodes[i].op, "*") == 0){
        if(isDigit(nodes[i].r)){
          ...
          if(const_num < -32768 || 32768 < const_num){
            ...
            add_op("mul", "r0", "label"+std::to_string(label_counter));
            label_counter++;
          }
          else{
            // Output2("muli\tr0,%d\n", atoi(nodes[i].r));
            add_op("muli", "r0", nodes[i].r);
          }
        }
        else{
          // Output2("mul\tr0,%d\n", get_var_addr(nodes[i].r));
          add_op("mul", "r0", std::to_string(get_var_addr(nodes[i].r)));
        }
      }
        ...
        if(isDigit(nodes[i].r)){
          ...
          if(const_num < -32768 || 32768 < const_num){
            ...
            add_op("div", "r0", "label"+std::to_string(label_counter));
            label_counter++;
          }
          else{
            // Output2("divi\tr0,%d\n", atoi(nodes[i].r));
            add_op("divi", "r0", nodes[i].r);
          }
        }
        else{
          // Output2("div\tr0,%d\n", get_var_addr(nodes[i].r));
          add_op("div", "r0", std::to_string(get_var_addr(nodes[i].r)));
        }
      }

      // Output2("store\tr0, %d\n", var_count + nodes[i].regi);
      add_op("store", "r0", std::to_string(var_count+ nodes[i].regi));
    }

    // 最後にメモリにためた演算結果をレジスタ0にロードしつつ０を返す
    // Output2("load\tr0, %d\n", var_count + nodes[i-1].regi);
    add_op("load", "r0", std::to_string(var_count)+std::to_string(nodes[i-1].regi));
    return 0;
  }
}
